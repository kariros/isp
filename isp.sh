...
# Скрипт с проверкой интерфейсов, правильным nftables и установкой hostname
# Внешний интерфейс жестко задан: ens160
# =====================================================
...
# Запрос имени интерфейса с проверкой (только для внутренних)
get_interface() {
    local prompt="$1"
    local iface
    while true; do
...
# Запрос и установка hostname
read -p "Введите желаемое имя хоста (например, ISP-1): " NEW_HOSTNAME
if [ -n "$NEW_HOSTNAME" ]; then
    echo "→ Установка hostname: $NEW_HOSTNAME"
    hostnamectl set-hostname "$NEW_HOSTNAME"
    hostname "$NEW_HOSTNAME"
    echo "✅ Hostname установлен. Перезайдите для применения в приглашении shell."
else
    echo "Имя хоста не изменено."
fi

# Внешний интерфейс жестко задан
EXT_IF="ens160"
echo "→ Внешний интерфейс: $EXT_IF (задан по умолчанию)"
